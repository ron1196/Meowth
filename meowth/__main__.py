...
async def create_raid_channel(type, pkmn, level, details, report_channel):
    ...
    if type == "exraid":
        ...
        name = _("{level}-").format(level=str(level))
        raid_channel_overwrite_list = report_channel.overwrites
        ...
        if raid_status:
            ...
            if guild_dict[message.guild.id]['raidchannel_dict'][message.channel.id]['active']:
                ...
                if "/maps" in message.content and "http" in message.content:
                    ...
                    await message.channel.send(content=_('Meowth! Someone has suggested a different location for the raid! Trainers {trainer_list}: make sure you are headed to the right place!').format(trainer_list=', '.join(otw_list)))
                    return
...
async def announce(ctx, *, announce=None):
    ...
    thumbnailUrl = None
    for w in announce:
        if (w.startswith('<') and w.endswith('>')) or w == '@everyone' or w == '@here':
            tags += w + ' '
        elif w.startswith("image:"):
            imageUrl = ":".join(w.split(":")[1:])
        elif w.startswith("thumb:"):
            thumbnailUrl = ":".join(w.split(":")[1:])
    announce = [x for x in announce if not ((x.startswith('<') and x.endswith('>')) or x == '@everyone' or x == '@here')]
    announce = [x for x in announce if not (x.startswith('thumb:') or x.startswith('image:'))]
    embeddraft = discord.Embed(colour=guild.me.colour, description=(' ').join(announce))
    ...
    if imageUrl:
        embeddraft.set_image(url=imageUrl)
    if thumbnailUrl:
        embeddraft.set_thumbnail(url=thumbnailUrl)
    draft = await channel.send(tags, embed=embeddraft)
    reaction_list = ['❔', '✅', '❎']
    owner_msg_add = ''
...
@Meowth.command(name='uptime', hidden=True)
    ...
    if (len(want_list) == 1) and ((len(added_list) == 1) or (len(spellcheck_dict) == 1) or (len(already_want_list) == 1)):
        if len(added_list) == 1:
            ...
            try:
                ...
                want_img_url = 'https://wiki.p-insurgence.com/images/0/09/722.png'
...
async def _wild(message, content):
    ...
    # BADDDD PROGRAMMING !!!!!!!!!!!!!!!!!!
    if message.channel.name == '100-iv':
        role100iv = discord.utils.get(message.guild.roles, name='100-iv')
        if role100iv == None:
            role100iv = ''
        else:
            role100iv = role100iv.mention + "-"
        wildreportcontent = f"{role100iv}{roletest}Wild {entered_wild.title()} reported by {message.author.name}! Details: {wild_details}"
    else:
        wildreportcontent = f"{roletest}Wild {entered_wild.title()} reported by {message.author.name}! Details: {wild_details}"
    # ENDDDD !!!!
    ...
    wildreportmsg = await message.channel.send(content=wildreportcontent, embed=wild_embed)
    await asyncio.sleep(0.25)
...
async def _raid(message, content):
    ...
    if raid == None:
        roletest = _("{pokemon} - ").format(pokemon=entered_raid)
    else:
    ...
    raidreport = await message.channel.send(content=_('{pokemon} raid reported by {member}! Details: {location_details}. Coordinate in {raid_channel}').format(pokemon=entered_raid.capitalize(), member=message.author.name, location_details=raid_details, raid_channel=raid_channel.mention), embed=report_embed)
    await asyncio.sleep(1)
    raidmsg = _("{roletest} raid reported by {member} in {citychannel}! Details: {location_details}. Coordinate here!\n\nClick the question mark reaction to get help on the commands that work in here.\n\nThis channel will be deleted five minutes after the timer expires.").format(roletest=roletest, pokemon=entered_raid.title(), member=message.author.mention, citychannel=message.channel.mention, location_details=raid_details)
...
async def _raidegg(message, content):
    ...
    if (egg_level > 5) or (egg_level == 0):
        ...
        raid_embed.set_thumbnail(url=raid_img_url)
        raidreport = await message.channel.send(content=_('Meowth! Level {level} raid egg reported by {member}! Details: {location_details}. Coordinate in {raid_channel}').format(level=egg_level, member=message.author.name, location_details=raid_details, raid_channel=raid_channel.mention))
        await asyncio.sleep(1)
...
async def _eggassume(args, raid_channel, author=None):
    ...
    try:
        ...
        egg_report = None
    if egglevel != 'EX':
        await raid_channel.send(_('{raidrole} in {location_details}').format(raidrole=raidrole, location_details=eggdetails['address']))
...
async def _eggtoraid(entered_raid, raid_channel, author=None):
    ...
    if egglevel.isdigit():
        hatchtype = 'raid'
        raidreportcontent = _('{pokemon} hatched! Details: {location_details}. Coordinate in {raid_channel}').format(pokemon=entered_raid.capitalize(), location_details=egg_address, raid_channel=raid_channel.mention)
        ...
        if guild_dict[raid_channel.guild.id]['configure_dict']['invite']['enabled']:
            ...
            invitemsgstr2 = ""
        raidreportcontent = "{pokemon} hatched! Details: {location_details}. {invitemsgstr} coordinate in {raid_channel}".format(pokemon=entered_raid.capitalize(), location_details=egg_address, invitemsgstr=invitemsgstr,raid_channel=raid_channel.mention)
...
async def starttime(ctx,*,start_time=""):
    ...
    if (start and now <= start) or timeset:
        ...
        await channel.send((f"Start Time: **{nextgroup}**" + (f", Trainers: {', '.join(trainer_list)}" if len(trainer_list) > 0 else "")))
...
async def new(ctx,*,content):
    ...
    if len(location_split) < 1:
        ...
        try:
            ...
            pass
        try:
            await oldreportmsg.edit(new_content=oldreportmsg.content, content=oldreportmsg.content)
        except:
            pass
        guild_dict[message.guild.id]['raidchannel_dict'][message.channel.id]['raidmessage'] = oldraidmsg.id
        guild_dict[message.guild.id]['raidchannel_dict'][message.channel.id]['raidreport'] = oldreportmsg.id
    ...
    if (checks.check_wantchannel(ctx) or checks.check_citychannel(ctx) or checks.check_raidchannel(ctx) or checks.check_eggchannel(ctx) or checks.check_exraidchannel(ctx)):
        ...
        async for message in channel.history(limit=500, reverse=True):
            if message.author.id == guild.me.id:
                c = _('Coordinate here')
                if c in message.content:
                    reportchannel = message.raw_channel_mentions[0] if len(message.raw_channel_mentions) > 0 else None
                    raidmessage = message
                    break
        if egg:
            raidtype = 'egg'
...
async def _counters(ctx, pkmn, user = None, weather = None, movesetstr = "Unknown Moveset"):
    ...
    weather = weather_match_list[weather_index]
    url += "strategies/CINEMATIC_ATTACK_WHEN_POSSIBLE/DEFENSE_RANDOM_MC?sort={sort}&".format(sort='TIME')
    ...
    async with ctx.typing():
        ...
        title = _('{pkmn} | {weather} | {sort} | {movesetstr}').format(pkmn=pkmn.title(),weather=weather_list[weather_index].title(),movesetstr=movesetstr, sort='TimeToWin')
...
async def _get_generic_counters(guild, pkmn, weather=None):
    ...
    url += "strategies/CINEMATIC_ATTACK_WHEN_POSSIBLE/DEFENSE_RANDOM_MC?sort=TIME&"
    url += "weatherCondition={weather}&dodgeStrategy=DODGE_REACTION_TIME&aggregation=AVERAGE".format(weather=weather)
    title_url = url.replace('https://fight', 'https://www')
    hyperlink_icon = 'https://i.imgur.com/fn9E5nb.png'
    pbtlr_icon = 'https://www.pokebattler.com/favicon-32x32.png'
    ...
    def clean(txt):
        return txt.replace('_', ' ').title()
    title = _('{pkmn} | {weather} | {sort} | Unknown Moveset').format(pkmn=pkmn.title(),weather=weather_list[index].title(), sort="TimeToWin")
    stats_msg = _("**CP:** {raid_cp}\n").format(raid_cp=raid_cp)
    stats_msg += _("**Weather:** {weather}\n").format(weather=clean(weather))
    stats_msg += _("**Attacker Level:** {atk_levels}").format(atk_levels=atk_levels)
    ...
    for moveset in data['byMove']:
        ...
        ctrs = moveset['defenders'][-6:]
        title = _('{pkmn} | {weather} | TimeToWin | {movesetstr}').format(pkmn=pkmn.title(), weather=weather_list[index].title(), movesetstr=movesetstr)
        ctrs_embed = discord.Embed(colour=guild.me.colour)
        ctrs_embed.set_author(name=title,url=title_url,icon_url=hyperlink_icon)
        ctrs_embed.set_thumbnail(url=img_url)
        ctrs_embed.set_footer(text=_('Results courtesy of Pokebattler'), icon_url=pbtlr_icon)
...
@Meowth.command(aliases=['l'], hidden=True)
...
async def _interest(ctx, tag=False, team=False):
    ...
    bulletpoint = '🔹'
    instinct_emoji = parse_emoji(ctx.guild, ':instinct:')
    mystic_emoji = parse_emoji(ctx.guild, ':mystic:')
    valor_emoji = parse_emoji(ctx.guild, ':valor:')
    
    def sortByTeam(trainer):
        author = ctx.guild.get_member(trainer[0])
        team = 'unknown'
        for role in author.roles:
            if role.name.lower() == 'mystic':
                team = 'mystic'
    ...
    maybe_list = []
    name_list = []
    for trainer in OrderedDict(sorted(trainer_dict.items(), key=sortByTeam)):
        user = ctx.guild.get_member(trainer)
        members_count = (f"({trainer_dict[trainer]['count']})  " if trainer_dict[trainer]['count'] > 1 else "") + mystic_emoji * int(trainer_dict[trainer]['party']['mystic']) + valor_emoji * int(trainer_dict[trainer]['party']['valor']) + instinct_emoji * int(trainer_dict[trainer]['party']['instinct'])
        want_list = f"({', '.join([x.title() for x in trainer_dict[trainer]['interest']])})" if trainer_dict[trainer].get('interest', None) else ""
        if trainer_dict[trainer]['status']['maybe']:
            name_list.append(f"\t {bulletpoint}**{user.display_name}** {members_count}  {want_list}")
            maybe_list.append(f"\t {bulletpoint}**{user.mention}** {members_count}  {want_list}")
            if user and team == False:
                ctx_maybecount += trainer_dict[trainer]['status']['maybe']
            elif user and team and trainer_dict[trainer]['party'][team]:
                ctx_maybecount += trainer_dict[trainer]['party'][team]

    if ctx_maybecount > 0:
        if (now.time() >= datetime.time(5, 0)) and (now.time() <= datetime.time(21, 0)) and (tag == True):
            maybe_exstr = _('{trainer_list}').format(trainer_list='\n'.join(maybe_list))
        else:
            maybe_exstr = _('{trainer_list}').format(trainer_list='\n'.join(name_list))
    listmsg = _(' {trainer_count} interested\n{including_string}').format(trainer_count=str(ctx_maybecount), including_string=maybe_exstr)
    return listmsg
...
async def _otw(ctx, tag=False, team=False):
    ...
    for trainer in OrderedDict(sorted(trainer_dict.items(), key=sortByTeam)):
        ...
        if trainer_dict[trainer]['status']['coming']:
            name_list.append(f"\t {bulletpoint}**{user.display_name}** {members_count}  {want_list}")
            otw_list.append(f"\t {bulletpoint}**{user.mention}** {members_count}  {want_list}")
            if user and team == False:
    ...
    if ctx_comingcount > 0:
        if (now.time() >= datetime.time(5, 0)) and (now.time() <= datetime.time(21, 0)) and (tag == True):
            otw_exstr = _('{trainer_list}').format(trainer_list='\n'.join(otw_list))
        else:
            otw_exstr = _('{trainer_list}').format(trainer_list='\n'.join(name_list))
    listmsg = _(' {trainer_count} on the way\n{including_string}').format(trainer_count=str(ctx_comingcount), including_string=otw_exstr)
    return listmsg
...
async def _waiting(ctx, tag=False, team=False):
    ...
    for trainer in OrderedDict(sorted(trainer_dict.items(), key=sortByTeam)):
        ...
        if trainer_dict[trainer]['status']['here']:
            name_list.append(f"\t {bulletpoint}**{user.display_name}** {members_count}  {want_list}")
            here_list.append(f"\t {bulletpoint}**{user.mention}** {members_count}  {want_list}")
            if user and team == False:
    ...
    if ctx_herecount > 0:
        if (now.time() >= datetime.time(5, 0)) and (now.time() <= datetime.time(21, 0)) and (tag == True):
            here_exstr = _('{trainer_list}').format(trainer_list='\n'.join(here_list))
        else:
            here_exstr = _('{trainer_list}').format(trainer_list='\n'.join(name_list))
    listmsg = _(' {trainer_count} waiting at the {raidtype}\n{including_string}!').format(trainer_count=str(ctx_herecount), raidtype=raidtype, including_string=here_exstr)
    return listmsg
...
async def _lobbylist(ctx, tag=False, team=False):
    ...
    for trainer in OrderedDict(sorted(trainer_dict.items(), key=sortByTeam)):
        user = ctx.guild.get_member(trainer)
        member_count = trainer_dict[trainer]['count']
        memberteams = ''
        for i in range(0, int(trainer_dict[trainer]['party']['mystic'])):
            memberteams+=mystic_emoji
        for i in range(0,int(trainer_dict[trainer]['party']['valor'])):
            memberteams+=valor_emoji
        for i in range(0,int(trainer_dict[trainer]['party']['instinct'])):
            memberteams+=instinct_emoji
        if (trainer_dict[trainer]['status']['lobby']) and user and team == False:
            ctx_lobbycount += trainer_dict[trainer]['status']['lobby']
            name_list.append('\t  ' + bulletpoint + ('**' + user.display_name) + '**' + (' ({membercount})  '.format(membercount=member_count) if member_count > 1 else '  ') + memberteams)
            lobby_list.append('\t  ' + bulletpoint + ('**' + user.mention) + '**' + (' ({membercount})  '.format(membercount=member_count) if member_count > 1 else '  ') + memberteams)
    ...
    if ctx_lobbycount > 0:
        if (now.time() >= datetime.time(5, 0)) and (now.time() <= datetime.time(21, 0)) and (tag == True):
            lobby_exstr = _('{trainer_list}').format(trainer_list='\n'.join(lobby_list))
        else:
            lobby_exstr = _('{trainer_list}').format(trainer_list='\n'.join(name_list))
    listmsg = _(' {trainer_count} in the lobby\n{including_string}').format(trainer_count=str(ctx_lobbycount), including_string=lobby_exstr)
    return listmsg
            ...
            try:
                ...
                if questauthor:
                    if len(questmsg) < 1500:
                        questmsg += ('\n🔹')
                        questmsg += _("**Reward**: {reward}, **Pokestop**: {location}([Directions])({url}), **Quest**: {quest}, **Reported By**: {author}").format(location=research_dict[questid]['location'].title(),quest=research_dict[questid]['quest'].title(),reward=research_dict[questid]['reward'].title(), author=questauthor.display_name, url=research_dict[questid].get('url',None))
                    else:
